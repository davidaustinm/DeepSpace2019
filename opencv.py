...
ETX = ';'
FS = '|'
EOT = 4 # End of Transmission marker
...
while cont:
    ...
    for c in contours:
        if cv2.contourArea(c) > 50:
            ...
            if (aspect_ratio < IDEAL_TARGET_RATIO * .50 
                ...
                continue # This is way off from a 5.5"x2" square's aspect ratio so skip it.
            ...
            #cv2.putText(res, str(round(angle, 1)), (int(tlx), int(tly)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
    ...
    for lt in left_targets:
        ...
        for rt in right_targets:
            ...
            if rx < lx:
                continue # Skip this target because with a lower X value it must be on the left.
            rcenter = (rx, ry)
